...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_name', '-f', type=str, default='pretrain_test_kmer.h5', help='Model name to load for prediction')
    ...
    parser.add_argument('--species','-s', type = str, default = 'mm10', help='The species we need to predict')
    parser.add_argument('--start_position','-sta', type = int, default = 0, help='Start position in the data to predict')
    parser.add_argument('--end_position','-end', type = int, default = 10000, help='End position in the data to predict')

    
    args = parser.parse_args()
    filename = args.file_name
    sequences = load_genome(args.species)
    ...
    deepsilencer.load_weights(filename)
            ...
            if indexes_encode[i][4] in type2line.keys():
                table.append([indexes_encode[i][1],indexes_encode[i][2]+1,indexes_encode[i][3]+1,\
                            '.',-int(indexes_encode[i][2])+int(indexes_encode[i][3]), 'DeepSilencer',type2line[indexes_encode[i][4]],\
                              type2tissue[indexes_encode[i][4]],'Mus musculus',args.species,\
